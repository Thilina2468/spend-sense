...
export default function SignUpForm({ isSignIn, setLoading, toggleForm, setError }: SignUpFormProps) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const key = name as keyof SignUpData;
  ...
  return (
    <div className={`w-full max-w-[28rem] transition-all duration-500 delay-1000 ease-in-out origin-center transform ${!isSignIn ? 'scale-100' : 'scale-0 fixed pointer-events-none'}`}>
      <form onSubmit={handleSignUp} className="bg-white p-[1rem] rounded-[1.5rem] w-full text-center">
        <h2 className="text-[2rem] font-bold mb-4">Sign up</h2>
        
        <div className="flex justify-center my-[2rem]">
          <div className={`${socialIconStyle} bg-[#DB4437] delay-[1.6s]`}><FaGoogle size={22} /></div>
        </div>
        ...
        <button onClick={handleSignUp} type="submit" className={btnStyle}>
          Sign up
        </button>
